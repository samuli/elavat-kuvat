...
const searchUrl = (lookfor, page = 1, topic, genre) => {
...
const frontPageUrl = () => {
  const fields = ['title','id','images', 'urls', 'recordPage', 'corporateAuthors', 'nonPresenterAuthors', 'year'].map(f => `field[]=${f}`).join('&');
  return `${base}/api/v1/search?${filters}&${fields}&limit=10&sort=last_indexed+desc}`;
};
...
export { searchUrl, recordUrl, autocompleteUrl, frontPageUrl, genreFacetsUrl, topicFacetsUrl, limit as searchLimit };
