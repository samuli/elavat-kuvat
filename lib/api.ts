...
export const base = isServer ? process.env.NEXT_PUBLIC_API_BASE_SERVER : clientBase;
const limit = 20;
...
const autocompleteUrl = (lookfor: string, limit: number): string => {
  ...
  return `${base}/api/v1/search?lookfor=${encodeURIComponent(
    lookfor
...
const facetsUrl = (
  ...
  return `${base}/api/v1/search?lookfor=${lookfor ? encodeURIComponent(lookfor) : ''
    }&${queryFilters}&${facets}&limit=0`;
};
const topicFacetsUrl = (lookfor?: string, topic?: string, genre?: string, daterange?: TDaterange): string => {
