  ...
  <ul className="">
    { facets.map(f => (
      <AppLink prefetch={true} key={`facet-map-${f.value}`} href={facetUrl(facet, f.value)}>
        <a>
        <li role="button" className="inline-flex flex-auto mr-2 mb-1 px-2 py-1 bg-gradient-to-b from-gray-100 to-gray-200 rounded-md text-xs font-medium subpixel-antialiased text-gray-800 uppercase. cursor-pointer ripple-bg-white whitespace-nowrap" key={`facet-${f.value}`}>
            {f.translated}
         </li>
        </a>
      </AppLink>
