...
import { autocompleteUrl, IRecord, ISearchResult } from '@/lib/api';
...
type AutocompleteProps = {
  ...
  onSearch: () => unknown;
  onRecordSelect: () => unknown;
};

const Autocomplete = ({ ref, onSearch, onRecordSelect }: AutocompleteProps): React.ReactElement => {
  ...
  const { isFetching } = useQuery<ISearchResult>(autocompleteUrl(lookfor || '', limit), {
    onSuccess,
    onError,
    enabled: !!lookfor,
  });
  ...
  const menuItemClasses = (active) =>
    clsx('p-1 hover:text-gray-100', active && 'bg-gradient-to-r from-pink-500 to-pink-400');
        ...
        {inputItems.map((item, index) => (
          <li
                    ...
                    <div className="w-10 h-8 overflow-hidden rounded-md">
                      {(item.record?.images || []).length > 0 && (
                        <div className="mr-1">
                          ...
                          <Image src={(item.record?.images || [])[0] || ''} width={40} />
                        </div>
                      )}
                    </div>
                    <div className="pl-1">
                      <div className="text-md overflow-clip line-clamp-1">{item.record.title}</div>
                    </div>
                  </div>
                </a>
              </AppLink>
                ...
                <a>
                  <div className="pl-2 my-2">
                    <div className="text-md">
                      Näytä kaikki haulla <span className="font-bold">{lookfor}</span>{' '}
            ...
            {item.type === 'NO_RESULTS' && (
              <div className="pl-2">
                <div className="text-md">
                  Ei tuloksia haulla <span className="font-bold">{item.lookfor}</span>
                </div>
              </div>
            )}
          </li>
